module  KGen::oracle_management{
    ...
    use KGen::poa_nft;
    use KGen::PoG_Consensus_01;
    // use KGen::key_coin;
    // use reward_contract::reward_contract_01;
    ...
    public fun get_oracle_reward_balance(oracle_addr: address): u64 acquires RewardResource {
        let oracle_reward_resource = borrow_global<RewardResource>(oracle_addr).reward_balance;
        oracle_reward_resource
    }

    #[view]
    public fun registered_oracle_count(): u64 acquires RegisteredOracles {
        let registered_oracles_vector: vector<address> = borrow_global<RegisteredOracles>(@KGen).oracle_addresses;
    ...
    public fun is_oracle_already_added( oracle_node: address): bool acquires RegisteredOracles{
        ...
        let oracle_addresses_vector = borrow_global<RegisteredOracles>(@KGen).oracle_addresses;
    ...
    public entry fun set_verification_status(_owner: &signer, oracle: address, status: bool) acquires RewardResource {
        assert!(exists<RewardResource>(oracle), ERESOURCE_NOT_EXISTS);

        let oracle_resource = borrow_global_mut<RewardResource>(oracle);
        oracle_resource.oracle.verified = status;
    }

    public entry fun set_oracle_status(owner: &signer, oracle_addr: address, status: bool) acquires RewardResource {
    ...
    public entry fun distribute_rewards(owner: &signer, base_reward: u64) acquires RegisteredOracles, RewardResource {
        ...
        while ( i < addr_vector_length ) {
            ...
            if ( is_oracle_verified(*oracle_address)) {
                ...
                let _reward_points = base_reward * reward_resource.staked_key * performance_factor;
    ...
    public entry fun associate_reward_resource(oracle_node: &signer) {
        ...
        assert!(!exists<RewardResource>(signer::address_of(oracle_node)), ERESOURCE_ALREADY_EXISTS);
    ...
    public entry fun update_performance_metrics(_owner: &signer, oracle_address: address, uptime: u64, accuracy: u64) acquires RewardResource {
    ...
    public entry fun add_oracle_address(owner: &signer, oracle_addr: address, token_name: String) acquires RegisteredOracles, RewardResource{
    ...
    public entry fun mint_nft(_owner: &signer, oracle_addrs: address, token_name: String) acquires RewardResource {
    ...
    public entry fun set_leader_node(owner: &signer, oracle_addrs: address) acquires LeaderNode, RewardResource {

        assert!( signer::address_of(owner) == @KGen, ECALLER_NOT_ADMIN);
        assert!( is_oracle_verified(oracle_addrs), EORACLE_NOT_VERIFIED);

        let _leader_node_addrs = borrow_global_mut<LeaderNode>(signer::address_of(owner)).leader_address;

        _leader_node_addrs = oracle_addrs;
    }

    public entry fun remove_oracle_address(owner: &signer, oracle_addrs: address) acquires RegisteredOracles {
        ...
        let oracle_addresses_vector = borrow_global<RegisteredOracles>(signer::address_of(owner));
        let addr_vector_length = vector::length(&oracle_addresses_vector.oracle_addresses);

        while ( i < addr_vector_length ) {

            let oracle_address = vector::borrow(&oracle_addresses_vector.oracle_addresses, i);
        ...
        {
            let reg_oracle_vector = borrow_global_mut<RegisteredOracles>(signer::address_of(owner));
            vector::swap_remove<address>(&mut reg_oracle_vector.oracle_addresses, oracle_index);
        };

        assert!(!is_oracle_already_added(oracle_addrs), EORACLE_NOT_REMOVED);
    }

    public entry fun revoke_poa_nft(_owner: &signer, oracle_addrs: address, token_name: String) acquires RewardResource {
    ...
    public fun test_init_module(owner: &signer) acquires LeaderNode, RegisteredOracles {
        ...
        assert!(exists<LeaderNode>(signer::address_of(owner)), ELEADER_NODE_NOT_EXISTS);
        assert!(exists<RegisteredOracles>(signer::address_of(owner)), EREGISTER_ORACLE_NOT_EXISTS);
        // assert!(exists<oracle_management_contract::oracle_management::AdminSigner>(signer::address_of(owner)));

        let leader_node_addr = borrow_global<LeaderNode>(signer::address_of(owner)).leader_address;

        let oracle_vector = borrow_global<RegisteredOracles>(signer::address_of(owner)).oracle_addresses;
    ...
    public fun test_add_oracle_address(owner: &signer, oracle1: &signer) acquires RegisteredOracles, RewardResource {
        init_module(owner);
    ...
    #[test(owner=@KGen, oracle1=@0x1)]
    public fun test_remove_oracle_address(owner: &signer, oracle1: &signer) acquires RegisteredOracles, RewardResource {
        init_module(owner);
        poa_nft::create_collection(owner);
        KGen::key_coin::deploy_module(owner, 100);
        ...
        let registered_oracles_vector: vector<address> = borrow_global<RegisteredOracles>(signer::address_of(owner)).oracle_addresses;
        let vector_length = vector::length(&registered_oracles_vector);
        ...
        registered_oracles_vector= borrow_global<RegisteredOracles>(signer::address_of(owner)).oracle_addresses;
    ...
    public fun test_is_oracle_verified(owner: &signer, oracle1: &signer) acquires RewardResource {
        ...
        assert!( !is_oracle_verified(signer::address_of(oracle1)), EORACLE_ALREADY_VERIFIED);
    ...
    #[test(owner=@KGen, oracle1=@0x1)]
    public fun test_list_registered_oracle(owner: &signer, oracle1: &signer) acquires RegisteredOracles, RewardResource{
        init_module(owner);
        poa_nft::create_collection(owner);
        KGen::key_coin::deploy_module(owner, 100);
    ...
    #[test(owner=@KGen, oracle1=@0x1)]
    public fun test_get_oracle_reward_balance(owner:&signer, oracle1: &signer) acquires RewardResource {
        init_module(owner);
    ...
    #[test(owner=@KGen, oracle1=@0x1,oracle2=@0x2,oracle3=@0x3, leader_node=@0x111 )]
    public fun test_end_to_end(owner: &signer, oracle1: &signer, leader_node: &signer, oracle3:&signer, oracle2:&signer ) acquires RegisteredOracles, RewardResource, LeaderNode {
        init_module(owner);
        poa_nft::create_collection(owner);
        KGen::key_coin::deploy_module(owner, 100);
        PoG_Consensus_01::test_init_module(owner);
        ...
        associate_reward_resource(oracle1);
        add_oracle_address(owner, signer::address_of(oracle1), string::utf8(b"oracle1"));
        assert!(KGen::key_coin::key_count(signer::address_of(oracle1)) == 100,EINVALID_STAKED_KEY_COUNT);

        associate_reward_resource(oracle2);
        add_oracle_address(owner, signer::address_of(oracle2), string::utf8(b"oracle2"));
        assert!(KGen::key_coin::key_count(signer::address_of(oracle2)) == 100,EINVALID_STAKED_KEY_COUNT);

        associate_reward_resource(oracle3);
        add_oracle_address(owner, signer::address_of(oracle3), string::utf8(b"oracle3"));
        assert!(KGen::key_coin::key_count(signer::address_of(oracle3)) == 100,EINVALID_STAKED_KEY_COUNT);
        ...
        PoG_Consensus_01::test_submit_score(owner, player_id, ranks, scores);

        // std::debug::print(&string::utf8(b"indx"));
        // std::debug::print(&PoG_Consensus_01::view_player_index(1));

        assert!(PoG_Consensus_01::view_player_index(0) == 0, EPLAYER_ID_NOT_VALID);
        assert!(PoG_Consensus_01::fetch_player_score(0) == 100, EPLAYER_ID_NOT_VALID);
