module KGen::PoG_Consensus_01 {
    ...
    public fun view_player_index(player_id: u64): u64 acquires PlayerArray {
        ...
        std::debug::print(&string::utf8(b"Invalid player Id"));
    ...
    #[view]
    fun is_pub_key_present(input_pub_key: vector<u8>): bool acquires PublicKeyVector {
    ...
    public fun check_threshold(signatures: vector<vector<u8>>): bool acquires PublicKeyVector {
        let pub_key_vector = borrow_global<PublicKeyVector>(@KGen).oracle_public_keys;
        let pub_key_length = vector::length(&pub_key_vector);
    ...
    fun verify_signature(message_hash: vector<u8>, signature: vector<u8>, pub_key: vector<u8>): bool {
    ...
    fun update_and_store_scores( player_id: u64, player_rank: u64, player_score: u64): u64 acquires PlayerArray {
        if(  is_player_exists(player_id)   ) {
            let player_index = view_player_index(player_id);
            let players_array = borrow_global_mut<PlayerArray>(@KGen).players_array;
            let player = vector::borrow_mut<PlayerScore>(&mut players_array, player_index);
            ...
            let vector_lenght = vector::length(&players_vector.players_array);
            vector::push_back<PlayerScore>(&mut players_vector.players_array, player_score_obj );
            return vector_lenght
        };
        0
    }

    public entry fun submit_score(_caller: &signer, message_hash: vector<u8>, signature_vector: vector<vector<u8>>, pub_key_vector: vector<vector<u8>>, player_id: vector<u64>, ranks: vector<u64>, scores: vector<u64>  ) acquires PublicKeyVector, PlayerArray {

        // assert!(signer::address_of(caller) == KGen::oracle_management::get_leader_address(), ECALLER_NOT_LEADER);
        assert!(vector::length(&player_id) == vector::length(&ranks), EINVALID_VECTOR_LENGTH);
        assert!(vector::length(&player_id) == vector::length(&scores), EINVALID_VECTOR_LENGTH);
        assert!(vector::length(&signature_vector) == vector::length(&pub_key_vector), EINVALID_VECTOR_LENGTH);
        ...
        while ( x < vector::length(&signature_vector) ) {
            ...
            assert!( verify_signature(message_hash, *signature, *pub_key), EINVALID_SIGNATURES);
    ...
    public entry fun test_submit_score(_caller: &signer, player_id: vector<u64>, ranks: vector<u64>, scores: vector<u64>) acquires PlayerArray {
        let i = 0;
        while ( i < vector::length(&player_id) ){
            let player_id = vector::borrow(&player_id, i);
            let player_rank = vector::borrow(&ranks, i);
    ...
    public entry fun add_oracle_public_key(owner: &signer, public_key: vector<u8>) acquires PublicKeyVector {
        assert!(signer::address_of(owner) == @KGen, ECALLER_NOT_ADMIN);

        let players_vector = borrow_global_mut<PublicKeyVector>(signer::address_of(owner));

        vector::push_back<vector<u8>>(&mut players_vector.oracle_public_keys, public_key );
    }

    public entry fun remove_oracle_public_key(owner: &signer, public_key: vector<u8>) acquires PublicKeyVector {
        assert!(signer::address_of(owner) == @KGen, ECALLER_NOT_ADMIN);
        assert!( is_pub_key_present(public_key), EPUB_KEY_NOT_PRESENT);

        let i = 0;
        let pub_key_vector = borrow_global_mut<PublicKeyVector>(@KGen).oracle_public_keys;
        let vector_length = vector::length(&pub_key_vector);

        while( i < vector_length) {
            let pub_key = vector::borrow(&pub_key_vector, i);
            if ( *pub_key == public_key ){
                vector::swap_remove<vector<u8>>(&mut pub_key_vector, i);
            };
            break
        };
    ...
    public fun test_test_submit_score(owner: &signer, ) acquires PlayerArray {
        ...
        //     i = i + 1;
        // };
        // std::debug::print(&string::utf8(b"Player score 1"));
        ...
        // assert!(fetch_player_score(0) == 300, EINVALID_PLAYER_SCORE);
        // assert!(fetch_player_score(1) == 200, EINVALID_PLAYER_SCORE);
