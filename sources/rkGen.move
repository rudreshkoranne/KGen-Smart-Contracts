module KGen::rkGen_token {
    ...
    entry fun init_module(admin: &signer) {
        assert!(signer::address_of(admin) == @KGen, error::invalid_argument(E_NOT_OWNER));
    ...
    public fun pseudo_init(admin: &signer) {
        assert!(signer::address_of(admin) == @KGen, error::invalid_argument(E_NOT_OWNER));
    ...
    public entry fun transfer(
        ...
        assert!(balance >= amount, error::permission_denied(E_INSUFFICIENT_BALANCE));
    ...
    public fun test_basic_flow(admin: &signer, oracle1: address) acquires ManagedFungibleAsset {
        ...
        let asset = get_metadata();
        assert!(primary_fungible_store::balance(oracle1, asset) == 100, 4);
        burn(admin, oracle1, 50);
    ...
    public fun test_transfer(
        ...
        let asset = get_metadata();
        assert!(primary_fungible_store::balance(addr1, asset) == 100, 4);

        transfer(admin, addr1, addr2, 50);
