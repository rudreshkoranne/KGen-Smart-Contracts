...
entry fun init_module(admin: &signer) {
    ...
    //   fun mint_keys(admin: &signer, to: address, amount: u64) acquires ManagedFungibleAsset, State {
    //     validate_not_paused();
    ...
    //       // Borrow the mutable State resource to access the `key_limit` field.
    //       let state = borrow_global_mut<State>(object::object_address(&asset));
    
    //      // Ensure the minting amount doesn't exceed the limit.
    //      assert!(amount <= state.key_limit, error::permission_denied(EKEY_LIMIT_EXCEEDED));
    
    //      // Mint and deposit the specified amount to the recipient.
    //       let to_wallet = primary_fungible_store::ensure_primary_store_exists(to, asset);
    //       let fa = fungible_asset::mint(&managed_fungible_asset.mint_ref, amount);
    //       fungible_asset::deposit_with_ref(&managed_fungible_asset.transfer_ref, to_wallet, fa);
    //        //transfer(admin: &signer,@KEYCoin)
    //    }
